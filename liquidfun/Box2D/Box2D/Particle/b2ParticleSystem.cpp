...
static const uint32 relativeTagBottomLeft = static_cast<uint32>((uint64)((1 << yShift) + ((~uint64(0)) << xShift)));
...
public:
	...
	bool operator()(const b2ParticleBodyContact& contact)
	{
		...
		// normal of the Body that generated the contact makes physical sense
...
template <typename T> T* b2ParticleSystem::RequestBuffer(T* buffer)
{
	if (!buffer)
	{
		...
		for (int32 i = 0; i < m_internalAllocatedCapacity; ++i)
		{
			char* b = (char*)(void*)(&buffer[i]);
