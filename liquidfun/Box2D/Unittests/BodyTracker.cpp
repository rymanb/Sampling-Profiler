...
static std::string stringRemoveChars(const std::string& str, char c)
{
	std::string result;
	for (auto ch : str) {
		if (ch != c) {
...
bool BodyTracker::ReadBaseline()
{
	...
	header = stringRemoveChars(header, '\r');
	...
	for (std::string str; std::getline(in, str); ) {
		...
		int32 field = 0;
		for (std::string tok; std::getline(buff, tok, ','); ++field) {
			if (field == 0){
